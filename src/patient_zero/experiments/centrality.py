...
def dfs(node, parent, BFS_tree, subtree_size):
    ...
    for neighbor in BFS_tree.neighbors(node): # Loops over neighbors in the BFS tree.
        if neighbor != parent:
            dfs(neighbor, node, BFS_tree, subtree_size) # Recursively calls itself with the new neighbor node.
            subtree_size[node] += subtree_size[neighbor] # Calculates the subtree size.
...
def rumor_centrality(cascade: nx.Graph):
    ...
    for node in list(cascade.nodes): # Loops through every node in the cascade and calculates the rumor score for that node.
        ...
        rumor_score = {}
        rumor_score[node] = math.factorial(cascade_size) / prod # Compute root's rumor centrality. R(root) = n! / prod.
        node_scores[node] = rumor_score[node] # Sets the nodes rumor score.
    return node_scores
