...
def playground():
    ...
    G = create_balanced_tree_graph(children, 2, graph_seed)
    print(G)
    ...
    infected_nodes, cascade_edges = ic(G, patient_zero, r0, max_size, seed=model_seed, expand=expand)
...
def test():
    ...
    path_lengths = nx.single_source_shortest_path_length(cascade, patient_zero)
    degree_result = distance_centrality(cascade)
    guess = max(degree_result, key=degree_result.get)
    diff = path_lengths.get(guess)

    print(f"cascade={cascade}, cm=degree r0={r0}, guess={guess}, patient_zero={patient_zero} diff={diff}")
