...
def run_ic_simulation(graph, patient_zero=0, r=0.3, max_steps=None):
...
def run_sir_simulation(graph, patient_zero=0, r=0.1, recovery=0.1, max_steps=None):
    start = time.perf_counter()
...
def main():
    ...
    for k in ks:
        ...
        infected_nodes, duration = run_ic_simulation(g)
        print(f"Nodes={len(g.nodes())}, Infected nodes: {len(infected_nodes)}, Time: {duration:.4f}s")

    print("\nSIR Model Simulation:")
    for k in ks:
        g = create_tree_graph(k, h)
        s, i, r, duration = run_sir_simulation(g, max_steps=10)
        print(
